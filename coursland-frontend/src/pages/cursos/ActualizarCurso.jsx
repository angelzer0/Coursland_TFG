...
const ActualizarCurso = () => {
  ...
  useEffect(() => {
    const fetchCurso = async () => {
      try {
        const response = await CursoService.obtenerCursoPorId(cursoId);
        setCurso({
          titulo: response.titulo,
          descripcion: response.descripcion,
          dificultad: response.dificultad,
          categoria: response.categoria,
          enlaces: response.enlaces,
          archivosAdjuntos: [] 
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await CursoService.actualizarCurso(cursoId, curso, token);
  ...
  return (
    <div className="container mt-4">
      <br />
      <h2 className="mb-4" style={{ color: 'white', textShadow: '2px 2px 2px rgba(0, 0, 0, 1.0)' }}>Actualizar Curso</h2>
      <form className="bg-dark" onSubmit={handleSubmit} style={{ color: 'white', padding: '40px', borderRadius: '15px' }}>
        ...
        <button type="submit" className="btn btn-primary" style={{ backgroundColor: '#d9773f', borderColor: '#233540' }}>Actualizar Curso</button>
      </form>
    </div>
  );
};
