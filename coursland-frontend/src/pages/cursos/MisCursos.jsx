...
const MisCursos = () => {
    ...
    useEffect(() => {
        async function fetchCursos() {
            try {
                ...
                const listaCursos = await CursoService.listarCursosPorUsuario(token); // Cambia el método aquí
    ...
    return (
        <div style={{ padding: '20px', borderRadius: '8px', maxWidth: '800px', margin: '0 auto' }}>
            <ToastContainer />
            <h2 style={{ marginBottom: '20px', textAlign: 'center', color: 'white', textShadow: '2px 2px 2px rgba(0, 0, 0, 1.0)' }}>Mis Cursos</h2>
                            ...
                            {currentCursos.map(curso => (
                                <tr key={curso.idCurso}>
                                    ...
                                    <td style={{ padding: '10px', backgroundColor: '#f2f2f2', border: '1px solid #ddd' }}>
                                        <Link to={`/actualizarcurso/${curso.idCurso}`}>
                                            <button className="btn btn-warning" style={{ marginRight: '10px', backgroundColor: '#ffc107', border: 'none' }}>Actualizar</button>
                                        </Link>
